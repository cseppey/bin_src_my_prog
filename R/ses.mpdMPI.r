...
foldslave <- function() {
  ...
  while (done != 1) {
    ...
    if (tag == 1) {
      ...
      run <- mpd(samp, taxaShuffle(dis), abundance.weighted = abundance.weighted)

      run <- switch(null.model,
        taxa.labels = mpd(samp, taxaShuffle(dis), abundance.weighted), 
        richness =  mpd(randomizeMatrix(samp, null.model = "richness"), dis, abundance.weighted), 
        frequency =  mpd(randomizeMatrix(samp, null.model = "frequency"), dis, abundance.weighted), 
        sample.pool =  mpd(randomizeMatrix(samp, null.model = "richness"), dis, abundance.weighted), 
        phylogeny.pool =  mpd(randomizeMatrix(samp, null.model = "richness"), taxaShuffle(dis), abundance.weighted), 
        independentswap =  mpd(randomizeMatrix(samp, null.model = "independentswap", iterations), dis, abundance.weighted),
        trialswap =  mpd(randomizeMatrix(samp, null.model = "trialswap", iterations), dis, abundance.weighted))
...
# container
mpd.rand <- matrix(0, nrow=runs, ncol=nrow(samp))
...
while (closed_slaves < n_slaves) { 
  ...
  # slave send a result
    ...
    mpd.rand[foldNumber,] <- message$run
  }
...
write.table(data.frame(ntaxa = specnumber(samp), mpd.obs, mpd.rand.mean,
                       mpd.rand.sd, mpd.obs.rank, mpd.obs.z,
                       mpd.obs.p = mpd.obs.rank/(runs + 1), runs = runs,
