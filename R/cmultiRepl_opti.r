function (X, label = 0, method = c("GBM", "SQ", "BL", "CZM",
          "user"), output = c("prop", "p-counts"), delta = 0.65, threshold = 0.5, 
          correct = TRUE, t = NULL, s = NULL, suppress.print = FALSE) 
{
  ...
  if (suppress.print == FALSE) {
    if ((correct == TRUE) & (corrected > 0)) {
      cat(paste("No. corrected values: ", corrected, "\n"))
    }
  }
